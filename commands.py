...
async def start_private_command(client: Client, message: Message):
    ...
    welcome_message = (
        f"🌟 𝗳𝗛𝗲𝘆 **{user_name}** 𝗱𝗲𝗮𝗿! 𝗪𝗲𝗹𝗰𝗼𝗺𝗲! 🌟\n\n"
        "𝗳𝗜'𝗺 𝗿𝗲𝗮𝗱𝘆 𝘁𝗼 𝗵𝗲𝗹𝗽 𝘆𝗼𝘂!\n"
        "𝗳𝗖𝗹𝗶𝗰𝗸 𝘁𝗵𝗲 '𝗛𝗲𝗹𝗽' 𝗯𝘂𝘁𝘁𝗼𝗻 𝗯𝗲𝗹𝗼𝘄 𝘁𝗼 𝘀𝗲𝗲 𝗮𝗹𝗹 𝗺𝘆 𝗰𝗼𝗺𝗺𝗮𝗻𝗱𝘀."
    )
    keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("✙Hᴅᴅ Mᴇ Iɴ Yᴏᴜʀ Gʀᴏᴜᴘ✙", url=f"https://t.me/{client.me.username}?startgroup=true")],
            [
...
async def top_users_command(client: Client, message: Message):
    ...
    if not top_users:
        await send_and_auto_delete_reply(message, text="𝗳😢 𝗡𝗼 𝘂𝘀𝗲𝗿𝘀 𝗮𝗿𝗲 𝗼𝗻 𝘁𝗵𝗲 𝗹𝗲𝗮𝗱𝗲𝗿𝗯𝗼𝗮𝗿𝗱 𝘆𝗲𝘁! 𝗕𝗲 𝘁𝗵𝗲 𝗳𝗶𝗿𝘀𝘁 𝗯𝘆 𝗯𝗲𝗶𝗻𝗴 𝗮𝗰𝘁𝗶𝘃𝗲! ✨\n\n**Powered By:** @asbhaibsr", parse_mode=ParseMode.MARKDOWN)
        return

    earning_messages = ["👑 **𝗳𝗧𝗼𝗽 𝗔𝗰𝘁𝗶𝘃𝗲 𝗨𝘀𝗲𝗿𝘀 - ✨ 𝗩𝗜𝗣 𝗟𝗲𝗮𝗱𝗲𝗿𝗯𝗼𝗮𝗿𝗱! ✨** 👑\n\n"]
    prizes = {
        1: "💰 ₹50", 2: "💸 ₹30", 3: "🎁 ₹20",
        4: f"🎬 1 𝗳𝗪𝗲𝗲𝗸 𝗣𝗿𝗲𝗺𝗶𝘂𝗺 𝗣𝗹𝗮𝗻 𝗼𝗳 @{ASFILTER_BOT_USERNAME}",
        5: f"🎬 3 𝗳𝗗𝗮𝘆𝘀 𝗣𝗿𝗲𝗺𝗶𝘂𝗺 𝗣𝗹𝗮𝗻 𝗼𝗳 @{ASFILTER_BOT_USERNAME}"
    }

    for i, user in enumerate(top_users[:5]):
        rank = i + 1
        user_name = user.get('first_name', 'Unknown User')
        username_str = f"@{user.get('username')}" if user.get('username') else f"𝗳𝗜𝗗: `{user.get('user_id')}`"
        message_count = user.get('message_count', 0)
        prize_str = prizes.get(rank, "🏅 𝗳𝗡𝗼 𝗣𝗿𝗶𝘇𝗲")

        group_info = ""
        last_group_id = user.get('last_active_group_id')
        last_group_title = user.get('last_active_group_title', '𝗳𝗨𝗻𝗸𝗻𝗼𝘄𝗻 𝗚𝗿𝗼𝘂𝗽')

        if last_group_id:
            try:
                chat_obj = await client.get_chat(last_group_id)
                if chat_obj.type == ChatType.PRIVATE:
                    group_info = f"   • 𝗳𝗔𝗰𝘁𝗶𝘃𝗲 𝗶𝗻: **[𝗣𝗿𝗶𝘃𝗮𝘁𝗲 𝗖𝗵𝗮𝘁](tg://user?id={user.get('user_id')})**\n"
                elif chat_obj.username:
                    group_info = f"   • 𝗳𝗔𝗰𝘁𝗶𝘃𝗲 𝗶𝗻: **[{chat_obj.title}](https://t.me/{chat_obj.username})**\n"
                else:
                    try:
                        invite_link = await client.export_chat_invite_link(last_group_id)
                        group_info = f"   • 𝗳𝗔𝗰𝘁𝗶𝘃𝗲 𝗶𝗻: **[{chat_obj.title}]({invite_link})**\n"
                    except Exception:
                        group_info = f"   • 𝗳𝗔𝗰𝘁𝗶𝘃𝗲 𝗶𝗻: **{chat_obj.title}** (𝗣𝗿𝗶𝘃𝗮𝘁𝗲 𝗚𝗿𝗼𝘂𝗽)\n"
            except Exception as e:
                logger.warning(f"Could not fetch chat info for group ID {last_group_id} for leaderboard: {e}")
                group_info = f"   • 𝗳𝗔𝗰𝘁𝗶𝘃𝗲 𝗶𝗻: **{last_group_title}** (𝗜𝗻𝗳𝗼 𝗡𝗼𝘁 𝗔𝘃𝗮𝗶𝗹𝗮𝗯𝗹𝗲)\n"
        else:
            group_info = "   • 𝗳𝗔𝗰𝘁𝗶𝘃𝗲 𝗶𝗻: **𝗡𝗼 𝗚𝗿𝗼𝘂𝗽 𝗔𝗰𝘁𝗶𝘃𝗶𝘁𝘆**\n"

        earning_messages.append(
            f"**{rank}.** 🌟 **{user_name}** ({username_str}) 🌟\n"
            f"   • 𝗳𝗧𝗼𝘁𝗮𝗹 𝗠𝗲𝘀𝘀𝗮𝗴𝗲𝘀: **{message_count} 💬**\n"
            f"   • 𝗳𝗣𝗼𝘁𝗲𝗻𝘁𝗶𝗮𝗹 𝗣𝗿𝗶𝘇𝗲: **{prize_str}**\n"
            f"{group_info}"
        )
    
    earning_messages.append(
        "\n_𝗳𝗧𝗵𝗶𝘀 𝘀𝘆𝘀𝘁𝗲𝗺 𝗿𝗲𝘀𝗲𝘁𝘀 𝗼𝗻 𝘁𝗵𝗲 𝗳𝗶𝗿𝘀𝘁 𝗼𝗳 𝗲𝘃𝗲𝗿𝘆 𝗺𝗼𝗻𝘁𝗵!_\n"
        "_𝗳𝗨𝘀𝗲 `/help` 𝘁𝗼 𝗸𝗻𝗼𝘄 𝘁𝗵𝗲 𝗚𝗿𝗼𝘂𝗽 𝗿𝘂𝗹𝗲𝘀._"
    )
    keyboard = InlineKeyboardMarkup(
        [
            [
...
async def stats_private_command(client: Client, message: Message):
    ...
    if len(message.command) < 2 or message.command[1].lower() != "check":
        await send_and_auto_delete_reply(message, text="𝗳𝗨𝗺𝗺, 𝘁𝗼 𝗰𝗵𝗲𝗰𝗸 𝘀𝘁𝗮𝘁𝘀, 𝗽𝗹𝗲𝗮𝘀𝗲 𝘁𝘆𝗽𝗲 𝗰𝗼𝗿𝗿𝗲𝗰𝘁𝗹𝘆! 𝗟𝗶𝗸𝗲 𝘁𝗵𝗶𝘀: `/stats check`. 😊", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    stats_text = (
        "📊 **𝗳𝗕𝗼𝘁 𝗦𝘁𝗮𝘁𝗶𝘀𝘁𝗶𝗰𝘀** 📊\n"
        f"• 𝗳𝗡𝘂𝗺𝗯𝗲𝗿 𝗼𝗳 𝗴𝗿𝗼𝘂𝗽𝘀 𝗜'𝗺 𝗶𝗻: **{unique_group_ids}** 𝗹𝗼𝘃𝗲𝗹𝘆 𝗴𝗿𝗼𝘂𝗽𝘀!\n"
        f"• 𝗳𝗧𝗼𝘁𝗮𝗹 𝘂𝘀𝗲𝗿𝘀 𝗜 𝗼𝗯𝘀𝗲𝗿𝘃𝗲𝗱: **{num_users}** 𝘀𝘄𝗲𝗲𝘁 𝘂𝘀𝗲𝗿𝘀!\n"
        f"• 𝗳𝗧𝗼𝘁𝗮𝗹 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀 𝗜 𝘀𝘁𝗼𝗿𝗲𝗱 (𝗢𝗹𝗱 𝗟𝗲𝗮𝗿𝗻𝗶𝗻𝗴): **{total_messages}** 𝘁𝗿𝗲𝗮𝘀𝘂𝗿𝗲 𝗼𝗳 𝗰𝗼𝗻𝘃𝗲𝗿𝘀𝗮𝘁𝗶𝗼𝗻𝘀! 🤩\n"
        f"• 𝗳𝗢𝘄𝗻𝗲𝗿-𝘁𝗮𝘂𝗴𝗵𝘁 𝗽𝗮𝘁𝘁𝗲𝗿𝗻𝘀: **{total_owner_taught}** 𝘂𝗻𝗶𝗾𝘂𝗲 𝗽𝗮𝘁𝘁𝗲𝗿𝗻𝘀!\n"
        f"• 𝗳𝗖𝗼𝗻𝘃𝗲𝗿𝘀𝗮𝘁𝗶𝗼𝗻𝗮𝗹 𝗽𝗮𝘁𝘁𝗲𝗿𝗻𝘀 𝗹𝗲𝗮𝗿𝗻𝗲𝗱: **{total_conversational_learned}** 𝘂𝗻𝗶𝗾𝘂𝗲 𝗽𝗮𝘁𝘁𝗲𝗿𝗻𝘀!\n\n"
...
async def list_groups_command(client: Client, message: Message):
    ...
    if message.from_user.id != OWNER_ID:
        await send_and_auto_delete_reply(message, text="𝗳𝗢𝗼𝗽𝘀! 𝗦𝗼𝗿𝗿𝘆 𝘀𝘄𝗲𝗲𝘁𝗶𝗲, 𝘁𝗵𝗶𝘀 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 𝗶𝘀 𝗼𝗻𝗹𝘆 𝗳𝗼𝗿 𝗺𝘆 𝗯𝗼𝘀𝘀. 𝗬𝗼𝘂 𝗱𝗼𝗻'𝘁 𝗵𝗮𝘃𝗲 𝗽𝗲𝗿𝗺𝗶𝘀𝘀𝗶𝗼𝗻. 🤷‍♀️", parse_mode=ParseMode.MARKDOWN)
        return

    groups = list(group_tracking_collection.find({}))
    if not groups:
        await send_and_auto_delete_reply(message, text="𝗳𝗜'𝗺 𝗻𝗼𝘁 𝗶𝗻 𝗮𝗻𝘆 𝗴𝗿𝗼𝘂𝗽 𝗿𝗶𝗴𝗵𝘁 𝗻𝗼𝘄. 𝗜'𝗺 𝗹𝗼𝗻𝗲𝗹𝘆, 𝗽𝗹𝗲𝗮𝘀𝗲 𝗮𝗱𝗱 𝗺𝗲! 🥺", parse_mode=ParseMode.MARKDOWN)
        return

    group_list_text = "📚 **𝗳𝗚𝗿𝗼𝘂𝗽𝘀 𝗜'𝗺 𝗜𝗻** 📚\n\n"
    for i, group in enumerate(groups):
        title = group.get("title", "𝗳𝗨𝗻𝗸𝗻𝗼𝘄𝗻 𝗚𝗿𝗼𝘂𝗽")
        ...
        try:
            ...
            if chat_obj.username:
                ...
                try:
                    ...
                    group_link_display = f" ([𝗳𝗜𝗻𝘃𝗶𝘁𝗲 𝗟𝗶𝗻𝗸]({invite_link}))"
                except Exception:
                    group_link_display = " (𝗳𝗣𝗿𝗶𝘃𝗮𝘁𝗲 𝗚𝗿𝗼𝘂𝗽)"
        except Exception as e:
            logger.warning(f"Could not fetch chat info for group {group_id}: {e}")
            group_link_display = " (𝗳𝗜𝗻𝗳𝗼 𝗡/𝗔)"

        group_list_text += (
            f"{i+1}. **{title}** (`{group_id}`){group_link_display}\n"
            f"   • 𝗳𝗝𝗼𝗶𝗻𝗲𝗱: {added_on}\n"
            f"   • 𝗳𝗠𝗲𝗺𝗯𝗲𝗿𝘀: {member_count}\n"
        )

    group_list_text += "\n_𝗳𝗧𝗵𝗶𝘀 𝗱𝗮𝘁𝗮 𝗶𝘀 𝗳𝗿𝗼𝗺 𝘁𝗵𝗲 𝘁𝗿𝗮𝗰𝗸𝗶𝗻𝗴 𝗱𝗮𝘁𝗮𝗯𝗮𝘀𝗲, 𝗶𝘁'𝘀 𝗮 𝘀𝗲𝗰𝗿𝗲𝘁!_ 🤫\n**Powered By:** @asbhaibsr"
...
async def leave_group_command(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        await send_and_auto_delete_reply(message, text="𝗳𝗣𝗹𝗲𝗮𝘀𝗲 𝗽𝗿𝗼𝘃𝗶𝗱𝗲 𝘁𝗵𝗲 𝗚𝗿𝗼𝘂𝗽 𝗜𝗗 𝘆𝗼𝘂 𝘄𝗮𝗻𝘁 𝗺𝗲 𝘁𝗼 𝗹𝗲𝗮𝘃𝗲. 𝗨𝘀𝗮𝗴𝗲: `/leavegroup -1001234567890` (𝗹𝗶𝗸𝗲 𝘁𝗵𝗶𝘀, 𝗱𝗮𝗿𝗹𝗶𝗻𝗴!)", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    try:
        ...
        await send_and_auto_delete_reply(message, text=f"𝗳𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝗹𝗲𝗳𝘁 𝗴𝗿𝗼𝘂𝗽 `{group_id}`, 𝗮𝗻𝗱 𝗮𝗹𝘀𝗼 𝗰𝗹𝗲𝗮𝗻𝗲𝗱 𝗮𝗹𝗹 𝗶𝘁𝘀 𝗱𝗮𝘁𝗮! 𝗕𝘆𝗲-𝗯𝘆𝗲! 👋", parse_mode=ParseMode.MARKDOWN)
        logger.info(f"Left group {group_id} and cleared its data.")

    except ValueError:
        await send_and_auto_delete_reply(message, text="𝗳𝗜𝗻𝘃𝗮𝗹𝗶𝗱 𝗴𝗿𝗼𝘂𝗽 𝗜𝗗 𝗳𝗼𝗿𝗺𝗮𝘁. 𝗣𝗹𝗲𝗮𝘀𝗲 𝗽𝗿𝗼𝘃𝗶𝗱𝗲 𝗮 𝘃𝗮𝗹𝗶𝗱 𝗻𝘂𝗺𝗲𝗿𝗶𝗰 𝗜𝗗. 𝗖𝗵𝗲𝗰𝗸 𝘁𝗵𝗲 𝗻𝘂𝗺𝗯𝗲𝗿𝘀! 😉", parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
        await send_and_auto_delete_reply(message, text=f"𝗳𝗔𝗻 𝗲𝗿𝗿𝗼𝗿 𝗼𝗰𝗰𝘂𝗿𝗿𝗲𝗱 𝘄𝗵𝗶𝗹𝗲 𝗹𝗲𝗮𝘃𝗶𝗻𝗴 𝘁𝗵𝗲 𝗴𝗿𝗼𝘂𝗽: {e}. 𝗢𝗵 𝗻𝗼! 😢", parse_mode=ParseMode.MARKDOWN)
...
async def clear_data_command(client: Client, message: Message):
    ...
    if message.from_user.id != OWNER_ID:
        await send_and_auto_delete_reply(message, text="𝗳𝗦𝗼𝗿𝗿𝘆, 𝗱𝗮𝗿𝗹𝗶𝗻𝗴! 𝗧𝗵𝗶𝘀 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 𝗶𝘀 𝗼𝗻𝗹𝘆 𝗳𝗼𝗿 𝗺𝘆 𝗯𝗼𝘀𝘀. 🤫", parse_mode=ParseMode.MARKDOWN)
        return

    if len(message.command) < 2:
        await send_and_auto_delete_reply(message, text="𝗳𝗛𝗼𝘄 𝗺𝘂𝗰𝗵 𝗱𝗮𝘁𝗮 𝘁𝗼 𝗰𝗹𝗲𝗮𝗻? 𝗧𝗲𝗹𝗹 𝗺𝗲 𝘁𝗵𝗲 𝗽𝗲𝗿𝗰𝗲𝗻𝘁𝗮𝗴𝗲, 𝗹𝗶𝗸𝗲: `/cleardata 10%` 𝗼𝗿 `/cleardata 100%`! 🧹", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    try:
        ...
        if not (1 <= percentage <= 100):
            await send_and_auto_delete_reply(message, text="𝗳𝗣𝗲𝗿𝗰𝗲𝗻𝘁𝗮𝗴𝗲 𝘀𝗵𝗼𝘂𝗹𝗱 𝗯𝗲 𝗯𝗲𝘁𝘄𝗲𝗲𝗻 1 𝗮𝗻𝗱 100. 𝗕𝗲 𝗮 𝗹𝗶𝘁𝘁𝗹𝗲 𝗰𝗮𝗿𝗲𝗳𝘂𝗹! 🤔", parse_mode=ParseMode.MARKDOWN)
            return
    except ValueError:
        await send_and_auto_delete_reply(message, text="𝗳𝗜𝗻𝘃𝗮𝗹𝗶𝗱 𝗽𝗲𝗿𝗰𝗲𝗻𝘁𝗮𝗴𝗲 𝗳𝗼𝗿𝗺𝗮𝘁. 𝗣𝗲𝗿𝗰𝗲𝗻𝘁𝗮𝗴𝗲 𝘀𝗵𝗼𝘂𝗹𝗱 𝗯𝗲 𝗶𝗻 𝗻𝘂𝗺𝗯𝗲𝗿𝘀, 𝗹𝗶𝗸𝗲 `10` 𝗼𝗿 `50`. 𝗧𝗿𝘆 𝗮𝗴𝗮𝗶𝗻!💖", parse_mode=ParseMode.MARKDOWN)
        return
...
async def delete_specific_message_command(client: Client, message: Message):
    ...
    if message.from_user.id != OWNER_ID:
        await send_and_auto_delete_reply(message, text="𝗳𝗢𝗼𝗽𝘀! 𝗦𝗼𝗿𝗿𝘆 𝘀𝘄𝗲𝗲𝘁𝗶𝗲, 𝘁𝗵𝗶𝘀 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 𝗶𝘀 𝗼𝗻𝗹𝘆 𝗳𝗼𝗿 𝗺𝘆 𝗯𝗼𝘀𝘀. 🤷‍♀️", parse_mode=ParseMode.MARKDOWN)
        return

    if len(message.command) < 2:
        await send_and_auto_delete_reply(message, text="𝗳𝗪𝗵𝗶𝗰𝗵 **𝘁𝗲𝘅𝘁 𝗺𝗲𝘀𝘀𝗮𝗴𝗲** 𝘁𝗼 𝗱𝗲𝗹𝗲𝘁𝗲, 𝗽𝗹𝗲𝗮𝘀𝗲 𝘁𝗲𝗹𝗹 𝗺𝗲! 𝗟𝗶𝗸𝗲: `/deletemessage hello` 𝗼𝗿 `/deletemessage '𝗵𝗼𝘄 𝗮𝗿𝗲 𝘆𝗼𝘂'` 👻", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if deleted_count > 0:
        await send_and_auto_delete_reply(message, text=f"𝗳𝗔𝘀 𝘆𝗼𝘂 𝗰𝗼𝗺𝗺𝗮𝗻𝗱, 𝗺𝘆 𝗺𝗮𝘀𝘁𝗲𝗿! 🧞‍♀️ 𝗜 𝗳𝗼𝘂𝗻𝗱 𝗮𝗻𝗱 𝗱𝗲𝗹𝗲𝘁𝗲𝗱 **{deleted_count}** **𝘁𝗲𝘅𝘁 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀** 𝗿𝗲𝗹𝗮𝘁𝗲𝗱 𝘁𝗼 '{search_query}'. 𝗡𝗼𝘄 𝘁𝗵𝗮𝘁 𝗶𝘀𝗻'𝘁 𝗽𝗮𝗿𝘁 𝗼𝗳 𝗵𝗶𝘀𝘁𝗼𝗿𝘆 𝗮𝗻𝘆𝗺𝗼𝗿𝗲! ✨", parse_mode=ParseMode.MARKDOWN)
        logger.info(f"Deleted {deleted_count} text messages with query: '{search_query}'.")
    else:
        await send_and_auto_delete_reply(message, text="𝗳𝗨𝗺𝗺, 𝗜 𝗱𝗶𝗱𝗻'𝘁 𝗳𝗶𝗻𝗱 𝗮𝗻𝘆 **𝘁𝗲𝘅𝘁 𝗺𝗲𝘀𝘀𝗮𝗴𝗲** 𝗶𝗻 𝗺𝘆 𝗱𝗮𝘁𝗮𝗯𝗮𝘀𝗲 𝘄𝗶𝘁𝗵 𝘆𝗼𝘂𝗿 𝗾𝘂𝗲𝗿𝘆. 𝗖𝗵𝗲𝗰𝗸 𝘁𝗵𝗲 𝘀𝗽𝗲𝗹𝗹𝗶𝗻𝗴? 🤔", parse_mode=ParseMode.MARKDOWN)
...
async def delete_specific_sticker_command(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        await send_and_auto_delete_reply(message, text="𝗳𝗛𝗼𝘄 𝗺𝗮𝗻𝘆 **𝘀𝘁𝗶𝗰𝗸𝗲𝗿𝘀** 𝘁𝗼 𝗱𝗲𝗹𝗲𝘁𝗲? 𝗧𝗲𝗹𝗹 𝗺𝗲 𝘁𝗵𝗲 𝗽𝗲𝗿𝗰𝗲𝗻𝘁𝗮𝗴𝗲, 𝗹𝗶𝗸𝗲: `/delsticker 10%` 𝗼𝗿 `delsticker 20%` 𝗼𝗿 `delsticker 40%`! 🧹", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if deleted_count > 0:
        await send_and_auto_delete_reply(message, text=f"𝗳𝗔𝘀 𝘆𝗼𝘂 𝗰𝗼𝗺𝗺𝗮𝗻𝗱, 𝗺𝘆 𝗺𝗮𝘀𝘁𝗲𝗿! 🧞‍♀️ 𝗜 𝗳𝗼𝘂𝗻𝗱 𝗮𝗻𝗱 𝗱𝗲𝗹𝗲𝘁𝗲𝗱 **{percentage}%** 𝘀𝘁𝗶𝗰𝗸𝗲𝗿𝘀. 𝗔 𝘁𝗼𝘁𝗮𝗹 𝗼𝗳 **{deleted_count}** 𝘀𝘁𝗶𝗰𝗸𝗲𝗿𝘀 𝗿𝗲𝗺𝗼𝘃𝗲𝗱. 𝗡𝗼𝘄 𝘁𝗵𝗮𝘁 𝗶𝘀𝗻'𝘁 𝗽𝗮𝗿𝘁 𝗼𝗳 𝗵𝗶𝘀𝘁𝗼𝗿𝘆 𝗮𝗻𝘆𝗺𝗼𝗿𝗲! ✨", parse_mode=ParseMode.MARKDOWN)
        logger.info(f"Deleted {deleted_count} stickers based on {percentage}% request.")
    else:
        await send_and_auto_delete_reply(message, text="𝗳𝗨𝗺𝗺, 𝗜 𝗱𝗶𝗱𝗻'𝘁 𝗳𝗶𝗻𝗱 𝗮𝗻𝘆 **𝘀𝘁𝗶𝗰𝗸𝗲𝗿** 𝗶𝗻 𝗺𝘆 𝗱𝗮𝘁𝗮𝗯𝗮𝘀𝗲. 𝗘𝗶𝘁𝗵𝗲𝗿 𝘁𝗵𝗲𝗿𝗲 𝗮𝗿𝗲 𝗻𝗼 𝘀𝘁𝗶𝗰𝗸𝗲𝗿𝘀, 𝗼𝗿 𝘁𝗵𝗲 𝗽𝗲𝗿𝗰𝗲𝗻𝘁𝗮𝗴𝗲 𝗶𝘀 𝘁𝗼𝗼 𝗹𝗼𝘄! 🤔", parse_mode=ParseMode.MARKDOWN)
...
async def clear_earning_command(client: Client, message: Message):
    ...
    await send_and_auto_delete_reply(message, text="💰 **𝗳𝗘𝗮𝗿𝗻𝗶𝗻𝗴 𝗱𝗮𝘁𝗮 𝘀𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝗰𝗹𝗲𝗮𝗿𝗲𝗱!** 𝗡𝗼𝘄 𝗲𝘃𝗲𝗿𝘆𝗼𝗻𝗲 𝘄𝗶𝗹𝗹 𝘀𝘁𝗮𝗿𝘁 𝗳𝗿𝗼𝗺 𝘇𝗲𝗿𝗼 𝗮𝗴𝗮𝗶𝗻! 😉", parse_mode=ParseMode.MARKDOWN)
...
async def restart_command(client: Client, message: Message):
    ...
    await send_and_auto_delete_reply(message, text="𝗳𝗢𝗸𝗮𝘆, 𝗱𝗮𝗿𝗹𝗶𝗻𝗴! 𝗜'𝗺 𝘁𝗮𝗸𝗶𝗻𝗴 𝗮 𝘀𝗵𝗼𝗿𝘁 𝗻𝗮𝗽 𝗻𝗼𝘄 𝗮𝗻𝗱 𝘁𝗵𝗲𝗻 𝗜'𝗹𝗹 𝗯𝗲 𝗯𝗮𝗰𝗸, 𝗰𝗼𝗺𝗽𝗹𝗲𝘁𝗲𝗹𝘆 𝗳𝗿𝗲𝘀𝗵 𝗮𝗻𝗱 𝗲𝗻𝗲𝗿𝗴𝗲𝘁𝗶𝗰! 𝗣𝗹𝗲𝗮𝘀𝗲 𝘄𝗮𝗶𝘁 𝗮 𝗹𝗶𝘁𝘁𝗹𝗲, 𝗼𝗸𝗮𝘆? ✨", parse_mode=ParseMode.MARKDOWN)
...
async def clear_all_dbs_command(client: Client, message: Message):
    ...
    if message.from_user.id != OWNER_ID:
        await send_and_auto_delete_reply(message, text="𝗳𝗧𝗵𝗶𝘀 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 𝗶𝘀 𝗼𝗻𝗹𝘆 𝗳𝗼𝗿 𝗺𝘆 𝗯𝗼𝘀𝘀. 🚫", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    await send_and_auto_delete_reply(
        message,
        text="⚠️ **𝗳𝗪𝗔𝗥𝗡𝗜𝗡𝗚:** 𝗔𝗿𝗲 𝘆𝗼𝘂 𝘀𝘂𝗿𝗲 𝘆𝗼𝘂 𝘄𝗮𝗻𝘁 𝘁𝗼 𝗱𝗲𝗹𝗲𝘁𝗲 **𝗮𝗹𝗹 𝗱𝗮𝘁𝗮** 𝗳𝗿𝗼𝗺 𝘆𝗼𝘂𝗿 𝗠𝗼𝗻𝗴𝗼𝗗𝗕 𝗗𝗮𝘁𝗮𝗯𝗮𝘀𝗲𝘀 (𝗠𝗲𝘀𝘀𝗮𝗴𝗲𝘀, 𝗕𝘂𝘁𝘁𝗼𝗻𝘀, 𝗧𝗿𝗮𝗰𝗸𝗶𝗻𝗴)?\n\n"
             "𝗳𝗧𝗵𝗶𝘀 𝗮𝗰𝘁𝗶𝗼𝗻 𝗶𝘀 **𝗶𝗿𝗿𝗲𝘃𝗲𝗿𝘀𝗶𝗯𝗹𝗲** 𝗮𝗻𝗱 𝗮𝗹𝗹 𝘆𝗼𝘂𝗿 𝗱𝗮𝘁𝗮 𝘄𝗶𝗹𝗹 𝗯𝗲 𝗹𝗼𝘀𝘁 𝗳𝗼𝗿𝗲𝘃𝗲𝗿.\n\n"
             "𝗳𝗖𝗵𝗼𝗼𝘀𝗲 𝗰𝗮𝗿𝗲𝗳𝘂𝗹𝗹𝘆!",
        reply_markup=keyboard,
        parse_mode=ParseMode.MARKDOWN
    )
    ...
    if len(message.command) > 1 and message.from_user.id == OWNER_ID:
        try:
            ...
            if target_user_id == client.me.id:
                await send_and_auto_delete_reply(message, text="𝗳𝗬𝗼𝘂 𝗰𝗮𝗻'𝘁 𝗱𝗲𝗹𝗲𝘁𝗲 𝗺𝘆 𝗱𝗮𝘁𝗮, 𝗯𝗼𝘀𝘀! 😅", parse_mode=ParseMode.MARKDOWN)
                return
        except ValueError:
            await send_and_auto_delete_reply(message, text="𝗳𝗪𝗿𝗼𝗻𝗴 𝗨𝘀𝗲𝗿 𝗜𝗗 𝗳𝗼𝗿𝗺𝗮𝘁. 𝗣𝗹𝗲𝗮𝘀𝗲 𝗽𝗿𝗼𝘃𝗶𝗱𝗲 𝗮 𝘃𝗮𝗹𝗶𝗱 𝗻𝘂𝗺𝗲𝗿𝗶𝗰 𝗜𝗗.", parse_mode=ParseMode.MARKDOWN)
            return
    elif len(message.command) > 1 and message.from_user.id != OWNER_ID:
        await send_and_auto_delete_reply(message, text="𝗳𝗬𝗼𝘂 𝗮𝗿𝗲 𝗻𝗼𝘁 𝗮𝘂𝘁𝗵𝗼𝗿𝗶𝘇𝗲𝗱 𝘁𝗼 𝘂𝘀𝗲 𝘁𝗵𝗶𝘀 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 𝘁𝗵𝗶𝘀 𝘄𝗮𝘆. 𝗧𝗵𝗶𝘀 𝗳𝗲𝗮𝘁𝘂𝗿𝗲 𝗶𝘀 𝗼𝗻𝗹𝘆 𝗳𝗼𝗿 𝗺𝘆 𝗯𝗼𝘀𝘀.", parse_mode=ParseMode.MARKDOWN)
        return
    else:
        target_user_id = message.from_user.id

    if not target_user_id:
        await send_and_auto_delete_reply(message, text="𝗳𝗜 𝗰𝗮𝗻'𝘁 𝗳𝗶𝗴𝘂𝗿𝗲 𝗼𝘂𝘁 𝘄𝗵𝗼𝘀𝗲 𝗱𝗮𝘁𝗮 𝘁𝗼 𝗱𝗲𝗹𝗲𝘁𝗲. 😕", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    try:
        ...
        if deleted_messages_count > 0 or deleted_earning_data > 0:
            if target_user_id == message.from_user.id:
                await send_and_auto_delete_reply(message, text=f"𝗳𝗪𝗼𝘄! ✨ 𝗜 𝗵𝗮𝘃𝗲 𝗱𝗲𝗹𝗲𝘁𝗲𝗱 𝘆𝗼𝘂𝗿 `{deleted_messages_count}` 𝗰𝗼𝗻𝘃𝗲𝗿𝘀𝗮𝘁𝗶𝗼𝗻 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀 𝗮𝗻𝗱 𝗲𝗮𝗿𝗻𝗶𝗻𝗴 𝗱𝗮𝘁𝗮. 𝗬𝗼𝘂 𝗮𝗿𝗲 𝗰𝗼𝗺𝗽𝗹𝗲𝘁𝗲𝗹𝘆 𝗳𝗿𝗲𝘀𝗵 𝗻𝗼𝘄! 😊", parse_mode=ParseMode.MARKDOWN)
                logger.info(f"User {target_user_id} successfully cleared their data.")
            else:
                await send_and_auto_delete_reply(message, text=f"𝗳𝗕𝗼𝘀𝘀'𝘀 𝗼𝗿𝗱𝗲𝗿! 👑 𝗜 𝗵𝗮𝘃𝗲 𝗱𝗲𝗹𝗲𝘁𝗲𝗱 `{deleted_messages_count}` 𝗰𝗼𝗻𝘃𝗲𝗿𝘀𝗮𝘁𝗶𝗼𝗻 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀 𝗮𝗻𝗱 𝗲𝗮𝗿𝗻𝗶𝗻𝗴 𝗱𝗮𝘁𝗮 𝗳𝗼𝗿 𝘂𝘀𝗲𝗿 `{target_user_id}`. 😉", parse_mode=ParseMode.MARKDOWN)
                logger.info(f"Owner {message.from_user.id} cleared data for user {target_user_id}.")
        else:
            if target_user_id == message.from_user.id:
                await send_and_auto_delete_reply(message, text="𝗳𝗬𝗼𝘂 𝗱𝗼𝗻'𝘁 𝗵𝗮𝘃𝗲 𝗮𝗻𝘆 𝗱𝗮𝘁𝗮 𝘀𝘁𝗼𝗿𝗲𝗱 𝘁𝗼 𝗱𝗲𝗹𝗲𝘁𝗲. 𝗠𝘆 𝗱𝗮𝘁𝗮𝗯𝗮𝘀𝗲 𝗶𝘀 𝗰𝗼𝗺𝗽𝗹𝗲𝘁𝗲𝗹𝘆 𝗲𝗺𝗽𝘁𝘆 𝗳𝗼𝗿 𝘆𝗼𝘂! 🤷‍♀️", parse_mode=ParseMode.MARKDOWN)
            else:
                await send_and_auto_delete_reply(message, text=f"𝗳𝗡𝗼 𝗱𝗮𝘁𝗮 𝗳𝗼𝘂𝗻𝗱 𝗳𝗼𝗿 𝘂𝘀𝗲𝗿 `{target_user_id}` 𝘁𝗼 𝗱𝗲𝗹𝗲𝘁𝗲.", parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
        await send_and_auto_delete_reply(message, text=f"𝗳𝗦𝗼𝗺𝗲𝘁𝗵𝗶𝗻𝗴 𝘄𝗲𝗻𝘁 𝘄𝗿𝗼𝗻𝗴 𝘄𝗵𝗶𝗹𝗲 𝗱𝗲𝗹𝗲𝘁𝗶𝗻𝗴 𝗱𝗮𝘁𝗮: {e}. 𝗢𝗵 𝗻𝗼! 😱", parse_mode=ParseMode.MARKDOWN)
...
async def start_group_command(client: Client, message: Message):
    ...
    welcome_message = (
        f"🌟 𝗳𝗛𝗲𝘆 **{user_name}** 𝗱𝗲𝗮𝗿! 𝗪𝗲𝗹𝗰𝗼𝗺𝗲! 🌟\n\n"
        "𝗳𝗜'𝗺 𝗿𝗲𝗮𝗱𝘆 𝘁𝗼 𝗹𝗶𝘀𝘁𝗲𝗻 𝗮𝗻𝗱 𝗹𝗲𝗮𝗿𝗻 𝗮𝗹𝗹 𝘁𝗵𝗲 𝗴𝗿𝗼𝘂𝗽 𝗰𝗼𝗻𝘃𝗲𝗿𝘀𝗮𝘁𝗶𝗼𝗻𝘀!\n"
        "𝗳𝗨𝘀𝗲 𝘁𝗵𝗲 `/settings` 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 𝘁𝗼 𝗺𝗮𝗻𝗮𝗴𝗲 𝗮𝗹𝗹 𝗴𝗿𝗼𝘂𝗽 𝘀𝗲𝘁𝘁𝗶𝗻𝗴𝘀."
    )
    keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("✙Hᴅᴅ Mᴇ Iɴ Yᴏᴜʀ Gʀᴏᴜᴘ✙", url=f"https://t.me/{client.me.username}?startgroup=true")],
...
async def open_settings_command(client: Client, message: Message):
    ...
    if not await is_admin_or_owner(client, message.chat.id, message.from_user.id):
        await send_and_auto_delete_reply(message, text="𝗳𝗧𝗵𝗶𝘀 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 𝗰𝗮𝗻 𝗼𝗻𝗹𝘆 𝗯𝗲 𝘂𝘀𝗲𝗱 𝗯𝘆 𝗺𝘆 𝗯𝗼𝘀𝘀 (𝗔𝗱𝗺𝗶𝗻/𝗢𝘄𝗻𝗲𝗿)! 🤷‍♀️", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    settings_message = (
        f"⚙️ **𝗳𝗚𝗿𝗼𝘂𝗽 𝗦𝗲𝘁𝘁𝗶𝗻𝗴𝘀: {message.chat.title}** 🛠️\n\n"
        "𝗳𝗛𝗲𝗹𝗹𝗼, 𝗕𝗼𝘀𝘀! 𝗬𝗼𝘂 𝗰𝗮𝗻 𝗰𝗼𝗻𝘁𝗿𝗼𝗹 𝘁𝗵𝗲 𝗴𝗿𝗼𝘂𝗽 𝗿𝘂𝗹𝗲𝘀 𝗮𝗻𝗱 𝗯𝗼𝘁 𝗳𝘂𝗻𝗰𝘁𝗶𝗼𝗻𝘀 𝗳𝗿𝗼𝗺 𝘁𝗵𝗲 𝗯𝘂𝘁𝘁𝗼𝗻𝘀 𝗯𝗲𝗹𝗼𝘄.\n"
        "𝗳𝗨𝘀𝗲𝗿𝘀 𝘄𝗵𝗼 𝗯𝗿𝗲𝗮𝗸 𝘆𝗼𝘂𝗿 𝗳𝗶𝗹𝘁𝗲𝗿 𝘀𝗲𝘁𝘁𝗶𝗻𝗴𝘀 𝘄𝗶𝗹𝗹 𝗿𝗲𝗰𝗲𝗶𝘃𝗲 𝘁𝗵𝗲 **𝗗𝗲𝗳𝗮𝘂𝗹𝘁 𝗣𝘂𝗻𝗶𝘀𝗵𝗺𝗲𝗻𝘁**.\n\n"
        f"**𝗳𝗗𝗲𝗳𝗮𝘂𝗹𝘁 𝗣𝘂𝗻𝗶𝘀𝗵𝗺𝗲𝗻𝘁:** {punishment_text}\n"
        "__𝗳𝗖𝗵𝗼𝗼𝘀𝗲 𝘄𝗵𝗮𝘁 𝗽𝘂𝗻𝗶𝘀𝗵𝗺𝗲𝗻𝘁 𝘁𝗼 𝗴𝗶𝘃𝗲 𝘁𝗼 𝗿𝘂𝗹𝗲-𝗯𝗿𝗲𝗮𝗸𝗲𝗿𝘀 𝗳𝗿𝗼𝗺 '𝗗𝗲𝗳𝗮𝘂𝗹𝘁 𝗣𝘂𝗻𝗶𝘀𝗵𝗺𝗲𝗻𝘁'.__"
    )
